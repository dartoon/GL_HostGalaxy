...
for l in range(3):
	...
	(a1,a2),(b1,b2)=findpt(hs)              #find one brightest point from two arces
	hs=hs.T
	...
	for m in range(len(hs)):
	   for n in range(len(hs)):
		cell[m+270,n+270]+=hs[m,n]           #put image in a larger frame
	for i in range(len(psf)):
   	   for j in range(len(psf)):
		cell[i+x1+270,j+y1+270]+=psf[i,j]*800
		cell[i+x2+270,j+y2+270]+=psf[i,j]*2000
