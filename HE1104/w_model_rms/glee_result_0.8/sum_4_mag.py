...
for m in range(4):
 for i in range(8):   
    f='samp{0}/test_mag/{1}_gal'.format(i*4+1+m+48*2,filename)
...
for m in range(4):
 for i in range(8):   
    f='samp{0}/test_mag/{1}_fix_n'.format(i*4+1+m+48*2,filename)    
    mag[i,:] = np.loadtxt(f)[:,0]              #be careful! not np.loadtxt(f)[0] 
 #print np.shape(mag)
 kk=np.reshape(mag,(1,-1))
 #print np.shape(kk),np.shape(kk[kk!=0])
 c[m]=np.mean(kk[kk!=0])
