...
for l in range(ln):
	...
	#a1=181
	#a2=181
	filename='../fits/WFI2033-{0}.fits'.format(l+1)   # take one image
	...
	cel1=np.zeros([600,600])
	cel2=np.zeros([600,600])
	cel3=np.zeros([600,600])
	cel4=np.zeros([600,600])
	...
	for m in range(len(hs)):
	   for n in range(len(hs)):
		cell[m+180,n+180]+=hs[m,n]           #put all image into a larger frame
	for i in range(len(psf)):
   	   for j in range(len(psf)):
		cel1[i+x1+180,j+y1+180]+=psf[i,j]
		cel2[i+x2+180,j+y2+180]+=psf[i,j]
		cel3[i+x3+180,j+y3+180]+=psf[i,j]
		cel4[i+x4+180,j+y4+180]+=psf[i,j]
	cell+=(cel1*323+cel2*492+cel3*867+cel4*467)#*4/5           #add psf
