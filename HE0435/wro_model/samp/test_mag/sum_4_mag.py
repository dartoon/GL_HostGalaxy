...
for m in range(4):
 for i in range(12):   
    f0='mag_HE{0}/{1}_gal'.format(i*4+1+m+48*0,filename)
    mag0[i,:] = np.loadtxt(f0)[:,0]  
    f1='mag_HE{0}/{1}_gal'.format(i*4+1+m+48*1,filename)
    mag1[i,:] = np.loadtxt(f1)[:,0]   
    f2='mag_HE{0}/{1}_gal'.format(i*4+1+m+48*2,filename)
...
for m in range(4):
 for i in range(12):   
    f0='mag_HE{0}/{1}_fix_n'.format(i*4+1+m+48*0,filename)
    mag0[i,:] = np.loadtxt(f0)[:,0]  
    f1='mag_HE{0}/{1}_fix_n'.format(i*4+1+m+48*1,filename)
    mag1[i,:] = np.loadtxt(f1)[:,0]   
    f2='mag_HE{0}/{1}_fix_n'.format(i*4+1+m+48*2,filename)
    mag2[i,:] = np.loadtxt(f2)[:,0] 
 kk0=np.reshape(mag0,(1,-1))
 kk1=np.reshape(mag1,(1,-1))
 kk2=np.reshape(mag2,(1,-1))
...
plt.ylim(-1,3)
plt.legend([la0,lc0],['n limited 1-4;l_eta=0.8','n fixed to be real'],loc=2)
plt.grid()
