...
for i in range(8):
    arc=pyfits.open('only{0}/test_mag/gal1.fits'.format(i+1),mode = 'update')
    dx= (arc[0].header[12]-arc[0].header[11])/30
    dy= (arc[0].header[14]-arc[0].header[13])/30
    re= ((dx**2+dy**2)/2)**0.5
    arc.flush()
    f='only{0}/test_mag/{1}_gal'.format(i+1,filename)
...
for i in range(8):
    ...
    re= ((dx**2+dy**2)/2)**0.5
    arc.flush()
    f='only{0}/test_mag/{1}_fix_n'.format(i+1,filename)
    dif =np.loadtxt(f)[:,1] *re/0.13 -6
    mag = np.loadtxt(f)[:,0]
    kk=dif
    whe=mag
